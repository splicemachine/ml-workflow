...
INSERT INTO FEATURESTORE.FEATURE_SET_VERSION (feature_set_id, feature_set_version, deployed, deploy_ts, create_ts, create_username)
SELECT feature_set_id, 1, deployed, deploy_ts, last_update_ts, last_update_username
...
ALTER TABLE FEATURESTORE.TRAINING_SET_FEATURE ADD CONSTRAINT fk_training_set_feature_feature_version FOREIGN KEY (FEATURE_ID, FEATURE_SET_ID, FEATURE_VERSION) REFERENCES FEATURESTORE.FEATURE_VERSION(FEATURE_ID, FEATURE_SET_ID, FEAUTRE_SET_VERSION) ON UPDATE NO ACTION ON DELETE NO ACTION

ALTER TABLE FEATURESTORE.FEATURE_STATS DROP PRIMARY KEY;
ALTER TABLE FEATURESTORE.FEATURE_STATS DROP FOREIGN KEY fk_feature_stats_feature;
ALTER TABLE FEATURESTORE.FEATURE_STATS ADD COLUMN FEATURE_SET_ID INTEGER;
ALTER TABLE FEATURESTORE.FEATURE_STATS ADD COLUMN FEATURE_SET_VERSION INTEGER;
UPDATE FEATURESTORE.FEATURE_STATS SET fs.feature_set_id = (SELECT DISTINCT feature_set_id FROM FEATURESTORE.FEATURE_VERSION fv WHERE fv.feature_id = fs.feature_id), fs.feature_set_version = 1
FROM FEATURESTORE.FEATURE_STATS fs
ALTER TABLE FEATURESTORE.FEATURE_VERSION ADD PRIMARY KEY(FEATURE_ID, FEATURE_SET_ID, FEATURE_SET_VERSION, LAST_UPDATE_TS);
ALTER TABLE FEATURESTORE.FEATURE_VERSION ADD CONSTRAINT fk_feature_stats_feature_version FOREIGN KEY (FEATURE_ID, FEATURE_SET_ID, FEATURE_SET_VERSION) REFERENCES FEATURESTORE.FEATURE_VERSION(FEATURE_ID, FEATURE_SET_ID, FEATURE_SET_VERSION) ON UPDATE NO ACTION ON DELETE NO ACTION;

CREATE TABLE FEATURESTORE.PIPELINE_OPS_TMP (
    FEATURE_SET_ID INTEGER NOT NULL,
    EXTRACT_UP_TO_TS TIMESTAMP,
    PRIMARY KEY(FEATURE_SET_ID)
);
INSERT INTO FEATURESTORE.PIPELINE_OPS_TMP (feature_set_id, extract_up_to_ts)
SELECT feature_set_id, extract_up_to_ts
FROM FEATURESTORE.PIPELINE_OPS;
DROP TABLE FEATURESTORE.PIPELINE_OPS;
RENAME TABLE FEATURESTORE.PIPELINE_OPS_TMP TO PIPELINE_OPS;

CREATE TABLE FEATURESTORE.PIPELINE_AGG_TMP (
    FEATURE_SET_ID INTEGER NOT NULL,
    FEATURE_NAME_PREFIX VARCHAR(128) NOT NULL,
    COLUMN_NAME VARCHAR(128),
    AGG_FUNCTIONS VARCHAR(50),
    AGG_WINDOWS VARCHAR(255),
    AGG_DEFAULT_VALUE DECIMAL,
    LAST_UPDATE_TS TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    LAST_UPDATE_USERNAME VARCHAR(128) NOT NULL DEFAULT CURRENT_USER,
    PRIMARY KEY(FEATURE_SET_ID, FEATURE_NAME_PREFIX)
);
INSERT INTO FEATURESTORE.PIPELINE_AGG_TMP (feature_set_id, feature_name_prefix, column_name, agg_functions, agg_windows, agg_default_value, last_update_ts, last_update_username)
SELECT feature_set_id, feature_name_prefix, column_name, agg_functions, agg_windows, agg_default_value, last_update_ts, last_update_username
FROM FEATURESTORE.PIPELINE_AGG;
DROP TABLE FEATURESTORE.PIPELINE_AGG;
RENAME TABLE FEATURESTORE.PIPELINE_AGG_TMP TO PIPELINE_AGG;

DROP TABLE FEATURESTORE.PIPELINE;
