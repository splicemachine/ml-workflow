...
class SageMakerDeploymentHandler(BaseRunHandler):
    ...
    def _retrieve_model_binary_stream_from_db(self) -> None:
        ...
        artifact_query: text = text(
            f'SELECT "binary" FROM {SqlArtifact.__table_schema_name__} WHERE '
            f'run_uuid=:run_uuid AND name=:name'
        )

        try:
            self.artifact: bytearray = self.Session.execute(artifact_query, {
                "run_uuid": run_id,
                "name": self.model_dir
            })[0][0]
            LOGGER.info(f"Retrieved Artifact: {self.artifact}")
        except IndexError:
            LOGGER.exception(
    ...
    def _deploy_model_to_sagemaker(self) -> None:
        ...
        self.update_task_in_db(
            info='Deploying model to SageMaker; Waiting for endpoint to become active'
        )  # Update Information
    ...
    def _cleanup(self) -> None:
        ...
        run_shell_command(('rm', '-r', f'{DOWNLOAD_PATH}/*'))
