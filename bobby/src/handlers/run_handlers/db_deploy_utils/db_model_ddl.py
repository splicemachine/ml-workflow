...
from shared.db.connection import (Converters, DatabaseFunctions, SQL,
                                  SQLAlchemyClient)
...
class DatabaseModelDDL:
    ...
    def __init__(self,
        ...
        self.model.add_metadata(
            # We split on "(" because columns like DECIMAL(5,2) and VARCHAR(3000) we only want the type
            Metadata.MODEL_VECTOR_STR,
            ', '.join([f'{name} {col_type.split("(")[0]}' for name, col_type in self.model.get_metadata(
    ...
    def create_model_deployment_table(self):
        ...
        if DatabaseFunctions.table_exists(table_name=self.table_name, schema_name=self.schema_name,
    ...
    def alter_model_table(self):
        ...
        if not DatabaseFunctions.table_exists(table_name=self.table_name, schema_name=self.schema_name,
                                              engine=SQLAlchemyClient().engine):
            raise Exception(
    ...
    def add_model_to_metadata_table(self):
        ...
        self.session.execute(
            SQL.add_database_deployed_metadata.format(
    ...
    def _register_training_set(self, key_vals: Dict[str, str]):
        ...
        if (view_id or 'None') == 'None' and (view_version or None) == 'None':  # may return 'None' or None
        ...
        self.session.execute(SQL.training_set_instance.format(**tsi))
    ...
    def _register_training_set_features(self, ts: TrainingSet, key_vals: Dict[str, str]):
        ...
        create_time = datetime.strptime(key_vals['splice.feature_store.training_set_create_time'],
                                        '%Y-%m-%dT%H:%M:%S.%f')
        ...
        l = self.session.query(FeatureSetVersion.feature_set_id, FeatureSetVersion.feature_set_version) \
            .join(t, (FeatureSetVersion.feature_set_id == t.c.feature_set_id) & (
                FeatureSetVersion.deploy_ts == t.c.deploy_ts)) \
            .subquery('l')
        features: List[Feature] = self.session.query(Feature, FeatureVersion.feature_set_id,
                                                     FeatureVersion.feature_set_version) \
            .join(FeatureVersion, Feature.feature_id == FeatureVersion.feature_id) \
            .join(l, (FeatureVersion.feature_set_id == l.c.feature_set_id) & (
                FeatureVersion.feature_set_version == l.c.feature_set_version)) \
    ...
    def _register_model_deployment(self, ts: TrainingSet, key_vals: Dict[str, str]):
        ...
        if table_exists:  # Update
            ...
            self.session.execute(
                SQL.update_feature_store_deployment.format(**deployment)
            )
        else:  # Insert
            self.logger.info("Adding new deployment to Feature Store metadata", send_db=True)
            self.session.execute(
                SQL.add_feature_store_deployment.format(**deployment)
            )
    ...
    def _validate_training_view(self, view_id: int, view_version: int):
        ...
        tvw: TrainingViewVersion = self.session.query(TrainingViewVersion) \
            .filter(
            (TrainingViewVersion.view_id == view_id) & (TrainingViewVersion.view_version == view_version)).first()
        if not tvw:
            raise Exception(
                f"The version of the training view (id:{view_id}, version:{view_version}) used for this run has been deleted."
    ...
    def register_feature_store_deployment(self):
        ...
        optional_ts_params = [
            'training_view_id', 'training_view_version', 'training_set_label', 'training_set_id',
            'training_set_version', 'training_set_name'
        ]
        training_set_params = [
        ...
        if len(params) >= len(required_ts_params):  # Run has associated training set.
            ...
            if key_vals.get('splice.feature_store.training_view_id', 'None') != 'None' and \
                    key_vals.get('splice.feature_store.training_view_version', 'None') != 'None':
                ...
                self._validate_training_view(int(key_vals['splice.feature_store.training_view_id']),
                                             int(key_vals['splice.feature_store.training_view_version']))
